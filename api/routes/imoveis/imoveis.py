...
@router.get(
    '/imoveis-disponiveis',
    summary='Consultar todos os imóveis que não estão alugados',
    status_code=status.HTTP_200_OK,
    response_model=list[ImovelOut]
)
async def listar_imoveis_dispiniveis(db_session: DatabaseDependency,
                                     current_user: UsuarioModel = Depends(get_current_user)):
    return await get_imovel_disponivel(db_session, current_user)
